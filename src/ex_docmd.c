    ...
    int         flags;
{
    ...
    do
    {
        ...
        if (p_verbose >= 15 && sourcing_name != NULL)
        {
            ...
            smsg((char_u *)"line %ld: %s", (long)sourcing_lnum, cmdline_copy);
            if (msg_silent == 0)
        ...
        if (did_throw)
        {
            ...
            switch (current_exception->type)
            {
                ...
                case ET_INTERRUPT:
                    break;
                default:
                    p = vim_strsave((char_u *)e_internal);
            }
    ...
    else
    {
        ...
        if ((getline_equal(fgetline, cookie, getsourceline)
            ...
            do_debug(getline_equal(fgetline, cookie, getsourceline)
                    ? (char_u *)"End of sourced file"
                    : (char_u *)"End of function");
    }
 ...
 */
        ...
        switch (*p)
        {
            ...
            case 'h':   if (p != ea.cmd || !checkforcmd(&p, "hide", 3) || *p == NUL || ends_excmd(*p))
                            break;
                        ea.cmd = p;
                        cmdmod.hide = TRUE;
                        continue;
 ...
 */
    ...
    {
        if (!ea.skip)
            errormsg = (char_u *)"E464: Ambiguous use of user-defined command";
        goto doend;
    }
 ...
 */
    ...
    if (!ea.skip)
    {
        if (sandbox != 0 && !(ea.argt & SBOXOK))
        {
            ...
            errormsg = (char_u *)e_sandbox;
            goto doend;
        }
        if (!curbuf->b_p_ma && (ea.argt & MODIFY))
        {
            /* Command not allowed in non-'modifiable' buffer */
            errormsg = (char_u *)e_modifiable;
            goto doend;
        }
        ...
        if (text_locked() && !(ea.argt & CMDWIN) && !IS_USER_CMDIDX(ea.cmdidx))
        {
            ...
            if (cmdwin_type != 0)
                errormsg = (char_u *)e_cmdwin;
            else
                errormsg = (char_u *)e_secure;
            goto doend;
        }
    ...
    if (!ea.skip && !ni)
    {
        ...
        if (!global_busy && ea.line1 > ea.line2)
        {
            if (msg_silent == 0)
            {
                ...
                {
                    errormsg = (char_u *)"E493: Backwards range given";
                    goto doend;
                }
    ...
    if (ea.cmdidx == CMD_write || ea.cmdidx == CMD_update)
    {
        if (*ea.arg == '>')                     /* append */
        {
            ...
            {
                errormsg = (char_u *)"E494: Use w or w>>";
                goto doend;
            }
    ...
    if ((ea.argt & COUNT) && VIM_ISDIGIT(*ea.arg)
            && (!(ea.argt & BUFNAME) || *(p = skipdigits(ea.arg)) == NUL || vim_iswhite(*p)))
    {
        n = getdigits(&ea.arg);
        ea.arg = skipwhite(ea.arg);
        if (n <= 0 && !ni && (ea.argt & ZEROR) == 0)
        {
            errormsg = (char_u *)e_zerocount;
            goto doend;
        }
    ...
    {
        errormsg = (char_u *)e_trailing;
        goto doend;
    }

    if (!ni && (ea.argt & NEEDARG) && *ea.arg == NUL)
    {
        errormsg = (char_u *)e_argreq;
        goto doend;
    }
                ...
                if (!skip)
                {
                    ...
                    if (searchit(curwin, curbuf, &pos,
                                *cmd == '?' ? BACKWARD : FORWARD,
                                (char_u *)"", 1L, SEARCH_MSG, i, (linenr_T)0, NULL) != FAIL)
                        lnum = pos.lnum;
                    else
                    {
                        cmd = NULL;
    ...
    exarg_T     *eap;
{
    ...
    if (eap->argt & RANGE)
    {
        ...
        {
            case ADDR_LINES:
                if (!(eap->argt & NOTADR) && eap->line2 > curbuf->b_ml.ml_line_count)
                    return (char_u *)e_invrange;
                break;
            case ADDR_ARGUMENTS:
...
#define ESCAPE_CHARS escape_chars
        ...
        if ((eap->usefilter || eap->cmdidx == CMD_bang) && vim_strpbrk(repl, (char_u *)"!") != NULL)
        {
            char_u      *l;
            ...
            if (n == 2)
            {
                ...
                if (!has_wildcards)
                    for (p = eap->arg; *p; ++p)
                    {
                        ...
                        {
                            *errormsgp = (char_u *)"E172: Only one file name allowed";
                            return FAIL;
                        }
                    }
    ...
    int forceit;
{
    ...
    if (!forceit && only_one_window() && ARGCOUNT > 1 && !arg_had_last && n >= 0 && quitmore == 0)
    {
        if (message)
        {
...
invalid_count:
        ...
        else if (STRNICMP(attr, "complete", attrlen) == 0)
        {
            ...
            if (parse_compl_arg(val, (int)vallen, compl, argt, compl_arg) == FAIL)
                return FAIL;
        }
    ...
    exarg_T     *eap;
{
    ...
    switch (*eap->arg)
    {
        ...
        default: EMSG2((char *)e_invarg2, eap->arg); return;
    }
    do_sleep(len);
}
    ...
    int         isabbrev;
{
    ...
    switch (do_map((*cmdp == 'n') ? 2 : (*cmdp == 'u'), eap->arg, mode, isabbrev))
    {
        case 1: EMSG((char *)e_invarg);
                break;
        case 2: EMSG(isabbrev ? (char *)e_noabbr : (char *)e_nomap);
                break;
    }
}
    ...
    exarg_T     *eap;
{
    ...
    if (redir_fd != NULL || redir_reg || redir_vname)
        redir_off = FALSE;
}
    ...
    int         silent;
{
    ...
    while ((!stuff_empty() || (!typebuf_typed() && typebuf.tb_len > 0)) && !got_int)
    {
        update_topline_cursor();
    ...
    *errormsg = NULL;
    ...
    else
    {
        ...
        case SPEC_HASH:         /* '#' or "#99": alternate file */
                ...
                *usedlen = (int)(s - src); /* length of what we expand */
                ...
                else
                {
                    ...
                    if (buf == NULL)
                    {
                        *errormsg = (char_u *)"E194: No alternate file name to substitute for '#'";
                        return NULL;
                    }
                    if (lnump != NULL)
                        *lnump = ECMD_LAST;
        ...
        case SPEC_AFILE:        /* file name for autocommand */
                ...
                if (result == NULL)
                {
                    *errormsg = (char_u *)"E495: no autocommand file name to substitute for \"<afile>\"";
                    return NULL;
                }
        ...
        case SPEC_ABUF:         /* buffer number for autocommand */
                if (autocmd_bufnr <= 0)
                {
                    *errormsg = (char_u *)"E496: no autocommand buffer number to substitute for \"<abuf>\"";
                    return NULL;
                }
        ...
        case SPEC_AMATCH:       /* match name for autocommand */
                ...
                if (result == NULL)
                {
                    *errormsg = (char_u *)"E497: no autocommand match name to substitute for \"<amatch>\"";
                    return NULL;
                }
                break;
        ...
        case SPEC_SFILE:        /* file name for ":so" command */
                ...
                if (result == NULL)
                {
                    *errormsg = (char_u *)"E498: no :source file name to substitute for \"<sfile>\"";
                    return NULL;
                }
                break;
        case SPEC_SLNUM:        /* line in file for ":so" command */
                ...
                {
                    *errormsg = (char_u *)"E842: no line number to use for \"<slnum>\"";
                    return NULL;
                }
    ...
    if (resultlen == 0 || valid != VALID_HEAD + VALID_PATH)
    {
        if (valid != VALID_HEAD + VALID_PATH)
            ...
            *errormsg = (char_u *)"E499: Empty file name for '%' or '#', only works with \":p:h\"";
        else
            *errormsg = (char_u *)"E500: Evaluates to an empty string";
        result = NULL;
    }
    else
    ...
    exarg_T     *eap;
{
    if (STRCMP(eap->arg, "mswin") == 0)
    {
        ...
        set_option_value((char_u *)"keymodel", 0L, (char_u *)"startsel,stopsel", 0);
    }
    ...
    exarg_T     *eap;
{
    ...
    else if ((STRNICMP(eap->arg, "none", 4) == 0 && (vim_iswhite(eap->arg[4]) || ends_excmd(eap->arg[4]))))
        end = eap->arg + 4;
    else
    {
