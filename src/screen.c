            ...
            FOR_ALL_WINDOWS(wp)
            {
                if (W_WINROW(wp) < msg_scrolled)
                {
                    ...
                    else
                    {
                        ...
                        if (W_WINROW(wp) + wp->w_height + W_STATUS_HEIGHT(wp) <= msg_scrolled)
                    ...
                    && !(dollar_vcol >= 0 && mod_bot == mod_top + 1))
            {
                ...
                else
                {
                    ...
                    if (xtra_rows < 0)
                    {
                        ...
                        {
                            check_for_delay(FALSE);
                            if (win_del_lines(wp, row, -xtra_rows, FALSE, FALSE) == FAIL)
                                mod_bot = MAXLNUM;
                            else
...
#define FIX_FOR_BOGUSCOLS \
    ...
    for (;;)
    {
        ...
        if (draw_state != WL_LINE)
        {
            ...
            if (draw_state == WL_NR - 1 && n_extra == 0)
            {
                ...
                if ((wp->w_p_nu || wp->w_p_rnu)
                        && (row == startrow || vim_strchr(p_cpo, CPO_NUMCOL) == NULL))
                {
        ...
        {
            SCREEN_LINE(screen_row, W_WINCOL(wp), col, -(int)W_WIDTH(wp), wp->w_p_rl);
        ...
        if (draw_state == WL_LINE && area_highlighting)
        {
            ...
            if (!n_extra)
            {
                ...
                {
                    if (shl_flag == FALSE
                            && ((cur != NULL && cur->priority > SEARCH_HL_PRIORITY) || cur == NULL))
                    {
                        shl = &search_hl;
                        shl_flag = TRUE;
                    }
        ...
        if (n_extra > 0)
        {
            ...
            else
            {
                ...
                if ((wp->w_p_rl ? (col <= 0) : (col >= W_WIDTH(wp) - 1)) && utf_char2cells(mb_c) == 2)
                {
                    c = '>';
                    mb_c = c;
                    mb_l = 1;
        ...
        else
        {
            ...
            if (wp->w_p_list && (c == 160 || (mb_utf8 && mb_c == 160)) && lcs_nbsp)
            {
                c = lcs_nbsp;
                if (area_attr == 0 && search_attr == 0)
                {
            ...
            if (!(chartab[c & 0xff] & CT_PRINT_CHAR))
            {
                ...
                if (c == TAB && (!wp->w_p_list || lcs_tab1))
                {
                    ...
                    else
                    {
                        ...
                        len = (tab_len * utf_char2len(lcs_tab2));
                        if (n_extra > 0)
                            len += n_extra - tab_len;
                        c = lcs_tab1;
        ...
        if (c == NUL)
        {
            ...
            if (((wp->w_p_cuc
                      ...
                      && (int)wp->w_virtcol < W_WIDTH(wp) * (row - startrow + 1) + v
                      && lnum != wp->w_cursor.lnum)
                    || draw_color_col)
                    && !wp->w_p_rl
                    )
         ...
         */
        if ((wp->w_p_rl ? (col < 0) : (col >= W_WIDTH(wp)))
                && (*ptr != NUL
                    || (wp->w_p_list && lcs_eol != NUL && p_extra != at_end_str)
                    || (n_extra != 0 && (c_extra != NUL || *p_extra != NUL))))
        {
            SCREEN_LINE(screen_row, W_WINCOL(wp), col - boguscols, (int)W_WIDTH(wp), wp->w_p_rl);
            boguscols = 0;
            ++row;
            ++screen_row;
            ...
            if (screen_cur_row == screen_row - 1 && W_WIDTH(wp) == Columns)
            {
                   ...
                   )
                {
                    ...
                    if (MB_BYTE2LEN(ScreenLines[LineOffset[screen_row - 1] + (Columns - 1)]) > 1)
                        out_char(' ');
                    else
 ...
 */
    static void
screen_line(row, coloff, endcol, clear_width, rlflag)
    int     row;
    int     coloff;
    int     endcol;
    int     clear_width;
    ...
    if (clear_width > 0)
    {
        ...
        if (col + coloff < Columns)
        {
            ...
            if (ScreenLines[off_to] != (schar_T)c
                    || (int)ScreenLinesUC[off_to] != (c >= 0x80 ? c : 0)
    ...
    win_T       *wp;
{
    ...
    else
    {
        ...
        else
        {
            ...
            for (i = 0; p[i] != NUL && clen >= this_ru_col - 1; i += utfc_ptr2len(p + i))
        ...
        if (get_keymap_str(wp, NameBuff, MAXPATHL)
            ...
            screen_puts(NameBuff, row, (int)(this_ru_col - STRLEN(NameBuff) - 1 + W_WINCOL(wp)), attr);
    ...
    int         attr;
{
    ...
    while (col < screen_Columns && (len < 0 || (int)(ptr - text) < len) && *ptr != NUL)
    {
        ...
        if (need_redraw || force_redraw_this)
        {
             ...
             * (utf_off2cells() may return 2 on the right halve). */
            if (clear_next_cell)
    ...
    colnr_T         mincol;     /* minimal column for a match */
{
    ...
    if (shl->lnum == lnum)
    {
        colnr_T start = posmatch->pos[bot].col == 0 ? 0 : posmatch->pos[bot].col - 1;
        colnr_T end = posmatch->pos[bot].col == 0 ? MAXCOL : start + posmatch->pos[bot].len;
    ...
    {
        {
            ...
            if ((attr & HL_BOLD) && T_MD != NULL)       /* bold */
                out_str(T_MD);
            else if (aep != NULL && t_colors > 1 && aep->ae_u.cterm.fg_color && cterm_normal_fg_bold)
    ...
    int         width;
{
    ...
    (void)vim_memset(ScreenLinesUC + off, 0, (size_t)width * sizeof(u8char_T));
    ...
    win_T       *wp;
{
    ...
    mch_memmove(ScreenLinesUC + off_to, ScreenLinesUC + off_from, wp->w_width * sizeof(u8char_T));
...
setcursor()
{
    ...
    {
        validate_cursor();
        windgoto(W_WINROW(curwin) + curwin->w_wrow, W_WINCOL(curwin) + (
                /* With 'rightleft' set and the cursor on a double-wide
                 * character, position it on the leftmost column. */
                curwin->w_p_rl ? ((int)W_WIDTH(curwin) - curwin->w_wcol - ((utf_ptr2cells(ml_get_cursor()) == 2 && vim_isprintc(gchar_cursor())) ? 2 : 1)) : curwin->w_wcol));
    }
}
    ...
    int         mayclear;
{
    ...
    if (screen_ins_lines(0, W_WINROW(wp) + row, line_count, (int)Rows, NULL) == FAIL)
    {
    ...
    int         mayclear;
{
    ...
    if (screen_del_lines(0, W_WINROW(wp) + row, line_count, (int)Rows, FALSE, NULL) == FAIL)
        return FAIL;
    ...
    int         del;
{
    ...
    if (scroll_region || W_WIDTH(wp) != Columns)
    {
        ...
        if (del)
            retval = screen_del_lines(W_WINROW(wp) + row, 0, line_count, wp->w_height - row, FALSE, wp);
        else
            retval = screen_ins_lines(W_WINROW(wp) + row, 0, line_count, wp->w_height - row, wp);
        if (scroll_region && (wp->w_width == Columns || *T_CSV != NUL))
            scroll_region_reset();
        return retval;
    }
