...
static char_u *bottommsg = (char_u *)"E555: at bottom of tag stack";
static char_u *topmsg = (char_u *)"E556: at top of tag stack";
    ...
    int         verbose;        /* print "tag not found" message */
{
    ...
    else
    {
        ...
        if (*tag != NUL && (type == DT_TAG || type == DT_SELECT || type == DT_JUMP))
        {
            {
                /*
        ...
        else
        {
            ...
            else                                /* go to other matching tag */
            {
                ...
                {
                    EMSG("E425: Cannot go before first matching tag");
                    skip_msg = TRUE;
                    cur_match = 0;
    ...
    for (;;)
    {
        ...
        if (new_tag || (cur_match >= num_matches && max_num_matches != MAXCOL) || other_name)
        {
            ...
            if (find_tags(name, &new_num_matches, &new_matches, flags, max_num_matches, buf_ffname) == OK && new_num_matches < max_num_matches)
                max_num_matches = MAXCOL; /* If less than max_num_matches found: all matches found. */
        ...
        {
            if (verbose)
                EMSG2("E426: tag not found: %s", name);
        }
        else
        {
            ...
            if (type == DT_SELECT || (type == DT_JUMP && num_matches > 1))
            {
                ...
                for (i = 0; i < num_matches && !got_int; ++i)
                {
                    ...
                    if (!new_tag && ((use_tagstack && i == tagstack[tagstackidx].cur_match)))
                        *IObuff = '>';
                    else
                        *IObuff = ' ';
                    vim_snprintf((char *)IObuff + 1, IOSIZE - 1,
                            "%2d %s ", i + 1, mt_names[matches[i][0] & MT_MASK]);
                    msg_puts(IObuff);
                    if (tagp.tagkind != NULL)
                        msg_outtrans_len(tagp.tagkind, (int)(tagp.tagkind_end - tagp.tagkind));
                    msg_advance(13);
                    msg_outtrans_len_attr(tagp.tagname,
                                       (int)(tagp.tagname_end - tagp.tagname), hl_attr(HLF_T));
                    msg_putchar(' ');
                    taglen_advance(taglen);
                    ...
                    if (command_end != NULL)
                    {
                        ...
                        while (*p && *p != '\r' && *p != '\n')
                        {
                            ...
                            if (p == tagp.tagkind
                                    || (p + 5 == tagp.tagkind && STRNCMP(p, "kind:", 5) == 0))
                            {
                                p = tagp.tagkind_end;
                                continue;
                            }
            ...
            if (cur_match >= num_matches)
            {
                ...
                {
                    if (num_matches == 1)
                        EMSG("E427: There is only one matching tag");
                    else
                        EMSG("E428: Cannot go beyond last matching tag");
                    skip_msg = TRUE;
                }
            ...
            if (type != DT_SELECT && type != DT_JUMP && (num_matches > 1 || ic) && !skip_msg)
            {
                ...
                sprintf((char *)IObuff, "tag %d of %d%s",
                                cur_match + 1,
                                num_matches,
                                max_num_matches != MAXCOL ? " or more" : "");
                if (ic)
                    STRCAT(IObuff, "  Using tag with different case!");
            ...
            if (i == NOTAGFILE)
            {
                ...
                EMSG2("E429: File \"%s\" does not exist", nofile_fname);
            }
            else
            {
    ...
    exarg_T     *eap UNUSED;
{
    ...
    for (i = 0; i < tagstacklen; ++i)
    {
        if (tagstack[i].tagname != NULL)
        {
            ...
            msg_outtrans(IObuff);
            msg_outtrans_attr(name, tagstack[i].fmark.fnum == curbuf->b_fnum ? hl_attr(HLF_D) : 0);
            vim_free(name);
        }
            ...
            if (orgpat.headlen)
            {
                ...
                if (tagp.tagname_end == NULL)
                {
                    if (vim_strchr(lbuf, NL) == NULL)
                    {
                        ...
                        {
                            verbose_enter();
                            MSG("Ignoring long line in tags file");
                            verbose_leave();
                        }
                        if (state != TS_LINEAR)
                        {
            ...
            {
                {
                    ...
                    is_current = test_for_current(tagp.fname, tagp.fname_end, tag_fname, buf_ffname);
                    {
...
#define ML_HELP_LEN 6
                    ...
                    if (mfp != NULL)
                    {
                         ...
                         */
                          for (i = ga_match[mtt].ga_len; --i >= 0 && !got_int; )
                          {
                              ...
                              if (mfp2->len == mfp->len
                                      && vim_memcmp(mfp2->match, mfp->match, (size_t)mfp->len) == 0)
                                  break;
                              line_breakcheck();
                          }
                        if (i < 0)
        ...
        if (line_error)
        {
            EMSG2("E431: Format error in tags file \"%s\"", tag_fname);
                EMSGN("Before byte %ld", (long)ftell(fp));
            stop_searching = TRUE;
            line_error = FALSE;
        }
        ...
        if (sort_error)
        {
            EMSG2("E432: Tags file not sorted: %s", tag_fname);
            sort_error = FALSE;
        }
    ...
    *pbuf_end = NUL;
    ...
    if (getfile_result <= 0)            /* got to the right file */
    {
        ...
        if (str > pbuf_end - 1) /* search command with nothing following */
        {
            ...
            if (do_search(NULL, pbuf[0], pbuf + 1, (long)1, search_options, NULL))
                retval = OK;
            else
            {
                int     found = 1;
                ...
                if (found == 0)
                {
                    EMSG("E434: Can't find tag pattern");
                ...
                else
                {
                    ...
                    {
                        MSG("E435: Couldn't find tag, just guessing!");
    ...
    char_u *pat;
{
    ...
    ret = find_tags(pat, &num_matches, &matches, TAG_REGEXP | TAG_NOIC, (int)MAXCOL, NULL);
    if (ret == OK && num_matches > 0)
    {
        for (i = 0; i < num_matches; ++i)
        {
            ...
            if (add_tag_field(dict, "name", tp.tagname, tp.tagname_end) == FAIL
                    ...
                    || add_tag_field(dict, "kind", tp.tagkind, tp.tagkind_end) == FAIL
