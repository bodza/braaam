        ...
        if (VIsual_active || redo_VIsual_busy)
        {
            if (VIsual_mode == Ctrl_V)  /* block mode */
            {
                ...
                if (!redo_VIsual_busy)
                {
                    ...
                    if (end > oap->end_vcol)
                    {
                        if (*p_sel == 'e' && start >= 1 && start - 1 >= oap->end_vcol)
    ...
    if (check && curwin->w_p_scb)
    {
        ...
        else if (curwin == old_curwin)
        {
            ...
            if ((curwin->w_buffer == old_buf)
                && (curwin->w_topline != old_topline || curwin->w_leftcol != old_leftcol))
            {
...
nv_addsub(cap)
    cmdarg_T    *cap;
{
    if (!checkclearopq(cap->oap) && do_addsub((int)cap->cmdchar, cap->count1) == OK)
        prep_redo_cmd(cap);
}
    ...
    cmdarg_T    *cap;
{
    ...
    if (ptr == NULL && (n = find_ident_under_cursor(&ptr,
                    (cmdchar == '*' || cmdchar == '#') ? FIND_IDENT|FIND_STRING : FIND_IDENT)) == 0)
    {
        clearop(cap->oap);
        return;
    }
    ...
    cmdarg_T        *cap;
{
    ...
    (void)normal_search(cap, cap->cmdchar, cap->searchbuf, (cap->arg ? 0 : SEARCH_MARK));
}
    ...
    cmdarg_T    *cap;
{
     ...
     */
    if ((cap->cmdchar == '[' && vim_strchr((char_u *)"{(*/#mM", cap->nchar) != NULL)
