    ...
    char_u *fname;
{
    ...
    ffname = FullName_save(fname, TRUE);         /* force expansion, get rid of symbolic links */
    if (ffname != NULL)
    {
    ...
    else
    {
        ...
        for (;;)
        {
            for (attempt = 0; attempt <= 3; ++attempt)
            {
                ...
                for (buf = firstbuf; buf != NULL; buf = buf->b_next)
                    if (buf->b_p_bl == find_listed && buflist_match(&regmatch, buf, FALSE) != NULL)
                    {
                        if (curtab_only)
                        {
    ...
    int dont_truncate;
{
    ...
    *p++ = '"';
    ...
    {
        vim_snprintf_add((char *)buffer, IOSIZE, "%s", (char *)no_lines_msg);
    }
    else if (p_ru)
    {
...
#define SPACE_FOR_ARGNR (IOSIZE - 10)  /* at least room for " - VIM" */
            if (curbuf->b_fname == NULL)
                vim_strncpy(buf, (char_u *)"[No Name]", SPACE_FOR_FNAME);
            else
            {
            ...
            if (curbuf->b_fname != NULL)
            {
                ...
                if (p == buf + off)
                    ...
                    vim_strncpy(buf + off, (char_u *)"help", (size_t)(SPACE_FOR_DIR - off - 1));
                else
                    *p = NUL;
    ...
    int keep_tabs;              /* keep current tabs, for ":tab drop file" */
{
    ...
    for (;;)
    {
        ...
        for (wp = firstwin; wp != NULL; wp = wpnext)
        {
            ...
            {
                if (P_HID(buf) || forceit || buf->b_nwindows > 1 || !bufIsChanged(buf))
                {
                    ...
                    if (firstwin == lastwin && (first_tabpage->tp_next == NULL || !had_tab))
                        use_firstwin = TRUE;
                    else
                    {
    ...
    exarg_T     *eap;
{
    ...
    {
        r = (P_HID(wp->w_buffer) || !bufIsChanged(wp->w_buffer) || autowrite(wp->w_buffer, FALSE) == OK);
        if (!win_valid(wp))
        {
    ...
    for (s = ml_get(lnum); *s != NUL; ++s)
    {
        if (prev == -1 || vim_isspace(prev))
        {
            ...
            if ((s[0] == 'v' || s[0] == 'V') && s[1] == 'i' && s[2] == 'm')
            {
                ...
                vers = getdigits(&e);
                if (*e == ':'
                        && (s[0] != 'V' || STRNCMP(skipwhite(e + 1), "set", 3) == 0)
                        && (s[3] == ':'
