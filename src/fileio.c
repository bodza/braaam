...
#if defined(HAVE_FCHOWN) /* sequent-ptx lacks fchown() */
...
#if defined(MACOS_CLASSIC) /* TODO: Is it need for MACOS_X? (Dany) */
    /*
    ...
    char_u      *dir_name;
{
    ...
    if (fnamencmp(dir_name, full_path, len) == 0)
    {
        p = full_path + len;
        {
            if (vim_ispathsep(*p))
                ++p;
#if (1) /* the path separator is always part of the path */
            else
                p = NULL;
#endif
        }
...
#if !defined(UNIX) /* for Unix mch_open() already set the permission */
    mch_setperm(to, perm);
#endif
#if defined(HAVE_ACL)
    mch_set_acl(to, acl);
