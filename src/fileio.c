 ...
 */
    ...
    /* Create a swap file now, so that other Vims are warned that we are editing this file.
    ...
    if (!read_buffer)
    {
                ...
                || (fd = mch_open((char *)fname, O_RDONLY | O_EXTRA, 0)) < 0))
        {
            ...
            msg_scroll = msg_save;
            if (fd < 0)
                EMSG("E200: *ReadPre autocommands made the file unreadable");
            else
                EMSG("E201: *ReadPre autocommands must not change current buffer");
            curbuf->b_p_ro = TRUE;      /* must use "w!" now */
            return FAIL;
        }
    }
...
retry:
    ...
    if (advance_fenc)
    {
        ...
        if (tmpname != NULL)
        {
            unlink((char *)tmpname);                /* delete converted file */
            vim_free(tmpname);
            tmpname = NULL;
        }
    }
    ...
    if (converted)
    {
        ...
        {
            /* Skip conversion when it's already done (retry for wrong "fileformat"). */
            if (tmpname == NULL)
            {
                ...
                if (tmpname == NULL)
                {
                    ...
                    if (fd < 0)
                    {
                        ...
                        EMSG("E202: Conversion made file unreadable!");
                        error = TRUE;
                        goto failed;
                    }
                    goto retry;
...
failed:
    ...
    if (!recoverymode)
    {
        ...
        if (!filtering && !(flags & READ_DUMMY))
        {
            ...
            {
                STRCAT(IObuff, "[character special]");
                c = TRUE;
            }
#endif
            if (curbuf->b_p_ro)
            ...
            if (notconverted)
            {
                STRCAT(IObuff, "[NOT converted]");
                c = TRUE;
            }
            else if (converted)
            {
                STRCAT(IObuff, "[converted]");
                c = TRUE;
            }
            if (conv_error != 0)
            {
                sprintf((char *)IObuff + STRLEN(IObuff),
                       "[CONVERSION ERROR in line %ld]", (long)conv_error);
                c = TRUE;
            }
            else if (illegal_byte > 0)
            {
                sprintf((char *)IObuff + STRLEN(IObuff),
                         "[ILLEGAL BYTE in line %ld]", (long)illegal_byte);
                c = TRUE;
            }
            else if (error)
            {
 ...
 * Returns name of the resulting converted file (the caller should delete it after reading it).
    ...
    int         *fdp;           /* in/out: file descriptor of file */
{
    ...
    else
    {
        ...
        *fdp = -1;
        if (eval_charconvert(fenc, (char_u *)ENC_DFLT, fname, tmpname) == FAIL)
    ...
    int             filtering;
{
    ...
    /* must init bw_conv_buf before jumping to "fail" */
...

    {
                                       ...
                                       )
        {
            ...
            msg_scroll = msg_save;
            if (nofile_err)
                EMSG("E676: No matching autocommands for acwrite buffer");
            ...
            if (!aborting())
                EMSG("E203: Autocommands deleted or unloaded buffer to be written");
            return FAIL;
        }
        ...
        if (buf->b_ml.ml_line_count != old_line_count)
        {
            ...
            else                                                /* less lines */
            {
                ...
                if (end < start)
                {
                    --no_wait_return;
                    msg_scroll = msg_save;
                    EMSG("E204: Autocommand changed number of lines in unexpected way");
                    return FAIL;
                }
            }
        }
            ...
            while (*dirp)
            {
                ...
                if (backup != NULL)
                {
                    ...
                    else
                    {
                         ...
                         * bits for the group same as the protection bits for others.
                         */
    ...
    if (converted && wb_flags == 0 && wfname == fname)
    {
        if (!forceit)
        {
                        ...
                        , perm < 0 ? 0666 : (perm & 0777))) < 0)
    {
         ...
         * read-only. In that case the unlink() will fail.
         */
        if (errmsg == NULL)
        {
            struct stat st;
...
restore_backup:
    ...
    if (wfname != fname)
    {
        ...
        if (end != 0)
        {
            if (eval_charconvert((char_u *)ENC_DFLT, fenc, wfname, fname) == FAIL)
            {
        ...
        if (errmsg == NULL)
        {
            if (write_info.bw_conv_error)
            {
                ...
                else
                {
                    ...
                    errmsg = alloc(300);
                    vim_snprintf((char *)errmsg, 300, "E513: write error, conversion failed in line %ld (make 'fenc' empty to override)", (long)write_info.bw_conv_error_lnum);
                }
            }
            else if (got_int)
                errmsg = (char_u *)e_interr;
    ...
    if (!filtering)
    {
        ...
        if (write_info.bw_conv_error)
        {
            STRCAT(IObuff, " CONVERSION ERROR");
            c = TRUE;
            if (write_info.bw_conv_error_lnum != 0)
                vim_snprintf_add((char *)IObuff, IOSIZE, " in line %ld;",
        ...
        {
            if (append)
                STRCAT(IObuff, shortmess(SHM_WRI) ? " [a]" : " appended");
            else
    ...
    if (*p_pm && dobackup)
    {
        ...
        if (backup != NULL)
        {
             ...
             */
            if (org == NULL)
                EMSG("E205: Patchmode: can't save original file");
            ...
            if (org == NULL
                    || (empty_fd = mch_open(org,
                                      O_CREAT | O_EXTRA | O_EXCL | O_NOFOLLOW,
                                        perm < 0 ? 0666 : (perm & 0777))) < 0)
              EMSG("E206: patchmode: can't touch empty original file");
            else
              close(empty_fd);
        }
        if (org != NULL)
     ...
     */
    if (!p_bk && backup != NULL && unlink((char *)backup) != 0)
        EMSG("E207: Can't delete backup file");
    ...
    off_t   nchars;
{
    ...
    else
    {
        ...
        if (nchars == 1)
            ...
            sprintf((char *)p,
#if defined(LONG_LONG_OFF_T)
                    "%lld characters", (long long)nchars
#else
                    /* Explicit typecast avoids warning on Mac OS X 10.6 */
                    "%ld characters", (long)nchars
#endif
                    );
    }
}
    ...
    struct stat         *st;
{
    if (buf->b_mtime_read != 0 && time_differs((long)st->st_mtime, buf->b_mtime_read))
    {
        ...
        MSG_ATTR("WARNING: The file has been changed since reading it!!!", hl_attr(HLF_E));
        ...
        else if (flags & (FIO_UCS4 | FIO_UTF16 | FIO_UCS2 | FIO_LATIN1))
        {
            ...
            for (wlen = 0; wlen < len; wlen += n)
            {
                if (wlen == 0 && ip->bw_restlen != 0)
                {
                     ...
                     * buf[] to get a full sequence.  Might still be too short! */
    ...
    char_u      *fenc;
{
    ...
    if (*fenc == NUL || STRCMP((char_u *)ENC_DFLT, fenc) == 0)
    {
        same_encoding = TRUE;
        fenc_flags = 0;
    }
    else
    {
        ...
        enc_flags = get_fio_flags((char_u *)ENC_DFLT);
    ...
    int     prepend_dot;        /* may prepend a '.' to file name */
{
    ...
    if (shortname)
    {
        ...
        /*
         * If the extension starts with '.', truncate the base name at 8 characters
         */
        else if (*ext == '.')
        {
    ...
    char_u      *to;
{
    ...
    while ((n = read_eintr(fd_in, buffer, BUFSIZE)) > 0)
        ...
        {
            errmsg = "E208: Error writing to \"%s\"";
            break;
        }
    ...
    close(fd_in);
    if (close(fd_out) < 0)
        errmsg = "E209: Error closing \"%s\"";
    if (n < 0)
    {
        errmsg = "E210: Error reading \"%s\"";
        to = from;
    }
    if (errmsg != NULL)
    {
    ...
    int         focus UNUSED;   /* called for GUI focus event */
{
     ...
     * set, we are in the middle of a save or being called recursively: ignore this buffer. */
    if (buf->b_ffname == NULL
                ...
                ))
    {
        ...
        else
        {
            ...
            {
                if (!buf_valid(buf))
                    EMSG("E246: FileChangedShell autocommand deleted buffer");
            ...
            {
                if (*reason == 'd')
                    mesg = "E211: File \"%s\" no longer available";
                else
                {
                    helpmesg = TRUE;
                    can_reload = TRUE;
                     ...
                     * checked out of CVS).  Always warn when the buffer was changed.
                     */
                    if (reason[2] == 'n')
                    {
                        mesg = "W12: Warning: File \"%s\" has changed and the buffer was changed in Vim as well";
                        mesg2 = "See \":help W12\" for more info.";
                    }
                    else if (reason[1] == 'h')
                    {
                        mesg = "W11: Warning: File \"%s\" has changed since editing started";
                        mesg2 = "See \":help W11\" for more info.";
                    }
                    else if (*reason == 'm')
                    {
                        mesg = "W16: Warning: Mode of file \"%s\" has changed since editing started";
                        mesg2 = "See \":help W16\" for more info.";
                    }
                    else
    ...
    {
        retval = 1;
        mesg = "W13: Warning: File \"%s\" has been created after editing started";
    ...
    int         orig_mode;
{
    ...
    if (prep_exarg(&ea, buf) == OK)
    {
        ...
        else
        {
            ...
            {
                EMSG2("E462: Could not prepare for reloading \"%s\"", buf->b_fname);
                saved = FAIL;
            }
        }
        ...
        if (saved == OK)
        {
            ...
            {
                if (!aborting())
                    EMSG2("E321: Could not reload \"%s\"", buf->b_fname);
    ...
    char_u      *name;
{
    ...
    if (i == AUGROUP_ERROR)     /* the group doesn't exist */
        EMSG2("E367: No such group: \"%s\"", name);
    else
    {
    ...
    int     have_group;     /* TRUE when group name was found */
{
    ...
    if (*arg == '*')
    {
        ...
        {
            EMSG2("E215: Illegal character after *: %s", arg);
            return NULL;
        }
        pat = arg + 1;
    }
    else
    {
        for (pat = arg; *pat && !vim_iswhite(*pat); pat = p)
        {
            ...
            {
                if (have_group)
                    EMSG2("E216: No such event: %s", pat);
                else
                    EMSG2("E216: No such group or event: %s", pat);
                return NULL;
            }
        }
    }
    ...
    for ( ; *pat; pat = (*endpat == ',' ? endpat + 1 : endpat))
    {
        ...
        if (*cmd != NUL)
        {
            ...
            if (ap == NULL)
            {
                ...
                {
                    EMSGN("E680: <buffer=%d>: invalid buffer number ", buflocal_nr);
                    return FAIL;
                }
    ...
    int         do_msg;     /* give message for no matching autocmds? */
{
    ...
    if (*arg == '*')
    {
        EMSG("E217: Can't execute autocommands for ALL events");
        return FAIL;
    }
...
win_found:
    ...
    else
    {
        ...
        if (win_valid(aco->save_curwin))
        {
             ...
             * it was changed, we are still the same window and the buffer is valid. */
    ...
    exarg_T     *eap;       /* command arguments */
{
    ...
    if (nesting == 10)
    {
        EMSG("E218: autocommand nesting too deep");
        goto BYPASS_AU;
    }
