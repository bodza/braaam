 ...
 */
typedef int_u u8char_T;      /* int is 32 bits */
...
typedef short_u disptick_T;             /* display tick type */
 ...
 */
typedef struct nr_trans nr_trans_T;
...
/*
 * Flags for the cs_lflags item in condstack_T.
 */
...
typedef struct
{
    ...
    union
    {
        number_T        v_number;       /* number value */
...
int lbr_chartabsize(char_u *line, char_u *s, colnr_T col);
...
void vim_str2nr(char_u *start, int *hexp, int *len, int dooct, int dohex, long *nptr, long_u *unptr);
int hex2nr(int c);
...
void do_errthrow(condstack_T *cstack, char_u *cmdname);
int do_intthrow(condstack_T *cstack);
...
void do_throw(condstack_T *cstack);
...
int cleanup_conditionals(condstack_T *cstack, int searched_cond, int inclusive);
void rewind_conditionals(condstack_T *cstack, int idx, int cond_type, int *cond_level);
...
void filemess(char_u *name, char_u *s, int attr);
...
void msg_add_fname(char_u *fname);
...
void home_replace(char_u *src, char_u *dst, int dstlen, int one);
char_u *home_replace_save(char_u *src);
...
char_u *string_convert(char_u *ptr, int *lenp);
...
int get_syntax_attr(colnr_T col, int keep_state);
...
int syn_get_id(win_T *wp, long lnum, colnr_T col, int trans, int keep_state);
