            ...
            while (frp != NULL)
            {
                if (frp->fr_win != oldwin && frp->fr_win != NULL
                        && (frp->fr_win->w_width > new_size
                            || frp->fr_win->w_width > oldwin->w_width - new_size - 1))
                {
                    do_equal = TRUE;
                    break;
                }
    ...
    int         vertical UNUSED;  /* split windows vertically if TRUE */
{
    ...
    else
    {
        ...
        maxcount = (curwin->w_height + curwin->w_status_height - (p_wh - p_wmh)) / (p_wmh + STATUS_HEIGHT);
    }
    ...
    int         trigger_leave_autocmds UNUSED;
{
    ...
    if (curwin->w_localdir != NULL)
    {
         ...
         * directory (unless that was done already) and change to the local directory. */
        if (globaldir == NULL)
        {
            char_u      cwd[MAXPATHL];
    ...
    int         height;
{
       ...
       )
    {
        ...
        if (sline < 0)
        {
             ...
             */
            wp->w_wrow = line_size;
            if (wp->w_wrow >= wp->w_height && (W_WIDTH(wp) - win_col_off(wp)) > 0)
            {
                wp->w_skipcol += W_WIDTH(wp) - win_col_off(wp);
                --wp->w_wrow;
                while (wp->w_wrow >= wp->w_height)
                {
                    wp->w_skipcol += W_WIDTH(wp) - win_col_off(wp) + win_col_off2(wp);
                    --wp->w_wrow;
                }
            }
            set_topline(wp, lnum);
    ...
    int         perr;
{
    ...
    {
        if (perr == TRUE)
            EMSGN("E802: Invalid ID: %ld (must be greater than or equal to 1)", id);
        return -1;
    }
