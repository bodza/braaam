                            ...
                            if (loop
                                || ((options & SEARCH_END)
                                    ? (lnum + regmatch.endpos[0].lnum < start_pos.lnum
                                        || (lnum + regmatch.endpos[0].lnum == start_pos.lnum
                                             && (int)regmatch.endpos[0].col - 1 + extra_col
                                                        <= (int)start_pos.col))
                                    : (lnum + regmatch.startpos[0].lnum < start_pos.lnum
                                        || (lnum + regmatch.startpos[0].lnum == start_pos.lnum
                                             && (int)regmatch.startpos[0].col + extra_col
                                                      <= (int)start_pos.col))))
                            {
                                match_ok = TRUE;
                                matchpos = regmatch.startpos[0];
...
again:
    ...
    for (n = 0; n < count; ++n)
    {
        if (do_searchpair((char_u *)"<[^ \t>/!]\\+\\%(\\_s\\_[^>]\\{-}[^/]>\\|$\\|\\_s\\=>\\)",
                    (char_u *)"",
                    (char_u *)"</[^>]*>", BACKWARD, (char_u *)"", 0, NULL, (linenr_T)0, 0L) <= 0)
        {
            curwin->w_cursor = old_pos;
            goto theend;
        }
    ...
    r = do_searchpair(spat, (char_u *)"", epat, FORWARD, (char_u *)"", 0, NULL, (linenr_T)0, 0L);
    ...
    int         move;
{
                              ...
                              SEARCH_KEEP + flag, RE_SEARCH, 0, NULL) != FAIL)
    {
        ...
        called_emsg = FALSE;
        nmatched = vim_regexec_multi(&regmatch, curwin, curbuf, pos.lnum, (colnr_T)0, NULL);
