    ...
    typval_T    *save_tv;
{
    ...
    if (vimvars[idx].vv_type == VAR_UNKNOWN)
    {
        ...
        if (HASHITEM_EMPTY(hi))
            EMSG2((char *)e_intern2, "restore_vimvar()");
        else
    ...
    exarg_T     *eap;
{
    ...
    if (*expr != '=' && !(vim_strchr((char_u *)"+-.", *expr) != NULL && expr[1] == '='))
    {
        /*
    ...
    char_u      *nextchars;
{
    ...
    {
        EMSG((char *)"E687: Less targets than List items");
        return FAIL;
    }
    if (var_count - semicolon > i)
    {
        EMSG((char *)"E688: More targets than List items");
        return FAIL;
    }
    ...
    while (*arg != ']')
    {
        ...
        if (*arg == ';')
        {
            ...
            arg = ex_let_one(skipwhite(arg + 1), &ltv, FALSE, (char_u *)"]", nextchars);
            clear_tv(&ltv);
            if (arg == NULL)
                return FAIL;
            break;
        ...
        {
            EMSG2((char *)e_intern2, "ex_let_vars()");
            return FAIL;
        }
    }
    ...
    if (*arg == '[')
    {
        ...
        for (;;)
        {
            ...
            else if (*p == ';')
            {
                if (*semicolon == 1)
                {
                    EMSG((char *)"Double ; in list of variables");
                    return NULL;
                }
                *semicolon = 1;
            }
    ...
    int         *first;
{
    ...
    for (hi = ht->ht_array; todo > 0 && !got_int; ++hi)
    {
        if (!HASHITEM_EMPTY(hi))
        {
            --todo;
            di = HI2DI(hi);
            if (empty || di->di_tv.v_type != VAR_STRING || di->di_tv.vval.v_string != NULL)
    ...
    int *first;
{
    ...
    list_hashtable_vars(&curbuf->b_vars->dv_hashtab, (char_u *)"b:", TRUE, first);

    sprintf((char *)numbuf, "%ld", (long)curbuf->b_changedtick);
    list_one_var_a((char_u *)"b:", (char_u *)"changedtick", VAR_NUMBER, numbuf, first);
}
...
list_win_vars(first)
    int *first;
{
    list_hashtable_vars(&curwin->w_vars->dv_hashtab, (char_u *)"w:", TRUE, first);
}
...
list_tab_vars(first)
    int *first;
{
    list_hashtable_vars(&curtab->tp_vars->dv_hashtab, (char_u *)"t:", TRUE, first);
}
    ...
    int *first;
{
    if (current_SID > 0 && current_SID <= ga_scripts.ga_len)
        list_hashtable_vars(&SCRIPT_VARS(current_SID), (char_u *)"s:", FALSE, first);
}
    ...
    int *first;
{
    if (current_funccal != NULL)
        list_hashtable_vars(&current_funccal->l_vars.dv_hashtab, (char_u *)"l:", FALSE, first);
}
                    ...
                    else
                    {
                        if (arg == arg_subsc && len == 2 && name[1] == ':')
                        {
                            switch (*name)
                            {
                                ...
                                default:
                                          EMSG2((char *)"E738: Can't list variables for %s", name);
                            }
                        }
                        else
                        {
    ...
    char_u      *op;            /* "+", "-", "."  or NULL*/
{
    ...
    if (*arg == '$')
    {
        ...
        len = get_env_len(&arg);
        if (len == 0)
            EMSG2((char *)e_invarg2, name - 1);
        else
        {
            if (op != NULL && (*op == '+' || *op == '-'))
                EMSG2((char *)e_letwrong, op);
            else if (endchars != NULL && vim_strchr(endchars, *skipwhite(arg)) == NULL)
                EMSG((char *)e_letunexp);
    ...
    else if (*arg == '&')
    {
        ...
        if (p == NULL || (endchars != NULL && vim_strchr(endchars, *skipwhite(p)) == NULL))
            EMSG((char *)e_letunexp);
        else
        {
            long        n;
    ...
    int         fne_flags;  /* flags for find_name_end() */
{
    ...
    *p = NUL;
    ...
    if (v == NULL && !quiet)
        EMSG2((char *)e_undefvar, lp->ll_name);
    *p = cc;
    if (v == NULL)
        return NULL;
    ...
    while (*p == '[' || (*p == '.' && lp->ll_tv->v_type == VAR_DICT))
    {
        ...
        {
            if (!quiet)
                EMSG((char *)"E689: Can only index a List or Dictionary");
            return NULL;
        }
        if (lp->ll_range)
        {
            if (!quiet)
                EMSG((char *)"E708: [:] must come last");
            return NULL;
        }
        ...
        else
        {
            ...
            if (*p == ':')
            {
                ...
                {
                    if (!quiet)
                        EMSG((char *)"E709: [:] requires a List value");
                    if (!empty1)
                        clear_tv(&var1);
                    return NULL;
                }
    ...
    char_u      *op;
{
        ...
        if (!check_changedtick(lp->ll_name))
        {
            ...
            *endp = NUL;
            if (op != NULL && *op != '=')
            {
                ...
                /* handle +=, -= and .= */
                if (get_var_tv(lp->ll_name, (int)STRLEN(lp->ll_name), &tv, TRUE, FALSE) == OK)
                {
    ...
    else if (lp->ll_range)
    {
        ...
        if (ri != NULL)
            EMSG((char *)"E710: List value has more items than target");
        else if (lp->ll_empty2
                ? (lp->ll_li != NULL && lp->ll_li->li_next != NULL)
                : lp->ll_n1 != lp->ll_n2)
            EMSG((char *)"E711: List value has not enough items");
    }
    else
    {
        /*
    ...
    int         skip;
{
    ...
    *errp = TRUE;       /* default: there is an error */
    ...
    {
        EMSG((char *)"E690: Missing \"in\" after :for");
        return fi;
    }
    ...
    exarg_T     *eap;
{
    ...
    if (fudi.fd_newkey != NULL)
    {
        ...
        EMSG2((char *)e_dictkey, fudi.fd_newkey);
    ...
    if (*startarg != '(')
    {
        EMSG2((char *)"E107: Missing parentheses: %s", eap->arg);
        goto end;
    }
    ...
    for ( ; lnum <= eap->line2; ++lnum)
    {
        ...
        if (get_func_tv(name, (int)STRLEN(name), &rettv, &arg,
                    eap->line1, eap->line2, &doesrange, !eap->skip, fudi.fd_dict) == FAIL)
        {
            failed = TRUE;
            break;
        }
    ...
    int         forceit;
{
    ...
    if (forceit)
        return OK;
    EMSG2((char *)"E108: No such variable: \"%s\"", name);
    return FAIL;
}
    ...
    int         lock;
{
    ...
    {
        EMSG((char *)"E743: variable nested too deep for (un)lock");
        return;
    }
    if (deep == 0)
        return;
        ...
        *arg = skipwhite(p + len);
        ...
        if (evaluate)
        {
            ...
            else if (rettv->v_type == VAR_LIST || var2.v_type == VAR_LIST)
            {
                ...
                else if (rettv->v_type != var2.v_type || (type != TYPE_EQUAL && type != TYPE_NEQUAL))
                {
                    if (rettv->v_type != var2.v_type)
                        EMSG((char *)"E691: Can only compare List with List");
                    else
                        EMSG((char *)"E692: Invalid operation for List");
                    clear_tv(rettv);
                    clear_tv(&var2);
                    return FAIL;
                }
                else
                {
                    ...
                    n1 = list_equal(rettv->vval.v_list, var2.vval.v_list, ic, FALSE);
                    if (type == TYPE_NEQUAL)
                        n1 = !n1;
                }
            }
            ...
            else if (rettv->v_type == VAR_DICT || var2.v_type == VAR_DICT)
            {
                ...
                {
                    if (rettv->v_type != var2.v_type)
                        EMSG((char *)"E735: Can only compare Dictionary with Dictionary");
                    else
                        EMSG((char *)"E736: Invalid operation for Dictionary");
                    clear_tv(rettv);
                    clear_tv(&var2);
                    return FAIL;
                }
                else
                {
                    ...
                    n1 = dict_equal(rettv->vval.v_dict, var2.vval.v_dict, ic, FALSE);
                    if (type == TYPE_NEQUAL)
                        n1 = !n1;
                }
            }
            ...
            else if (rettv->v_type == VAR_FUNC || var2.v_type == VAR_FUNC)
            {
                ...
                {
                    if (rettv->v_type != var2.v_type)
                        EMSG((char *)"E693: Can only compare Funcref with Funcref");
                    else
                        EMSG((char *)"E694: Invalid operation for Funcrefs");
                    clear_tv(rettv);
                    clear_tv(&var2);
                    return FAIL;
                }
                else
                {
                    ...
                    if (rettv->vval.v_string == NULL || var2.vval.v_string == NULL)
                        n1 = FALSE;
                    else
        ...
        *arg = skipwhite(*arg + 1);
        ...
        if (evaluate)
        {
            ...
            if (use_float)
            {
                ...
                {
                    EMSG((char *)"E804: Cannot use '%' with Float");
                    return FAIL;
                }
                rettv->v_type = VAR_FLOAT;
                rettv->vval.v_float = f1;
    ...
    int         verbose;        /* give error messages */
{
    ...
    {
        if (verbose)
            EMSG((char *)"E695: Cannot index a Funcref");
        return FAIL;
    }
    else if (rettv->v_type == VAR_FLOAT)
    {
        if (verbose)
            EMSG((char *)e_float_as_string);
        return FAIL;
    }
    ...
    int         evaluate;
{
    ...
    {
        if (rettv != NULL)
            EMSG2((char *)"E112: Option name missing: %s", *arg);
        return FAIL;
    }
    ...
    *option_end = NUL;
    ...
    {
        if (rettv != NULL)
            EMSG2((char *)"E113: Unknown option: %s", *arg);
        ret = FAIL;
    }
    else if (rettv != NULL)
    {
    ...
    int         evaluate;
{
    ...
    if (*p != '"')
    {
        EMSG2((char *)"E114: Missing quote: %s", *arg);
        return FAIL;
    }
    ...
    int         evaluate;
{
    ...
    if (*p != '\'')
    {
        EMSG2((char *)"E115: Missing quote: %s", *arg);
        return FAIL;
    }
    ...
    int         evaluate;
{
    ...
    *arg = skipwhite(*arg + 1);
    while (**arg != ']' && **arg != NUL)
    {
        ...
        if (**arg != ',')
        {
            EMSG2((char *)"E696: Missing comma in List: %s", *arg);
            goto failret;
        }
        *arg = skipwhite(*arg + 1);
    }
    ...
    if (**arg != ']')
    {
        EMSG2((char *)"E697: Missing end of List ']': %s", *arg);
failret:
        if (evaluate)
            list_free(l, TRUE);
        return FAIL;
    ...
    int     recurse;    /* Free Lists and Dictionaries recursively. */
{
    ...
    for (item = l->lv_first; item != NULL; item = l->lv_first)
    {
        ...
        if (recurse || (item->li_tv.v_type != VAR_LIST && item->li_tv.v_type != VAR_DICT))
            clear_tv(&item->li_tv);
        vim_free(item);
    }
    vim_free(l);
...
garbage_collect()
{
    ...
    {
        verb_msg((char_u *)"Not enough memory to set references, garbage collection aborted!");
    }
    ...
    int     recurse;    /* Free Lists and Dictionaries recursively. */
{
    ...
    for (hi = d->dv_hashtab.ht_array; todo > 0; ++hi)
    {
        if (!HASHITEM_EMPTY(hi))
        {
            ...
            if (recurse || (di->di_tv.v_type != VAR_LIST && di->di_tv.v_type != VAR_DICT))
                clear_tv(&di->di_tv);
            vim_free(di);
            --todo;
        }
    ...
    dictitem_T *org;
{
    ...
    di = (dictitem_T *)alloc((unsigned)(sizeof(dictitem_T) + STRLEN(org->di_key)));
    if (di != NULL)
    {
    ...
    dictitem_T  *item;
{
    ...
    if (HASHITEM_EMPTY(hi))
        EMSG2((char *)e_intern2, "dictitem_remove()");
    else
    ...
    if (copy != NULL)
    {
        ...
        for (hi = orig->dv_hashtab.ht_array; todo > 0 && !got_int; ++hi)
        {
            if (!HASHITEM_EMPTY(hi))
            {
                ...
                if (deep)
                {
                    if (item_copy(&HI2DI(hi)->di_tv, &di->di_tv, deep, copyID) == FAIL)
                    {
                        vim_free(di);
                        break;
                    }
    ...
    int         evaluate;
{
    ...
    *arg = skipwhite(*arg + 1);
    while (**arg != '}' && **arg != NUL)
    {
        ...
        if (**arg != ':')
        {
            EMSG2((char *)"E720: Missing colon in Dictionary: %s", *arg);
            clear_tv(&tvkey);
            goto failret;
        }
        if (evaluate)
        ...
        *arg = skipwhite(*arg + 1);
        ...
        if (evaluate)
        {
            ...
            if (item != NULL)
            {
                EMSG2((char *)"E721: Duplicate key in Dictionary: \"%s\"", key);
                clear_tv(&tvkey);
                clear_tv(&tv);
                goto failret;
            }
        ...
        if (**arg != ',')
        {
            EMSG2((char *)"E722: Missing comma in Dictionary: %s", *arg);
            goto failret;
        }
        *arg = skipwhite(*arg + 1);
    }
    ...
    if (**arg != '}')
    {
        EMSG2((char *)"E723: Missing end of Dictionary '}': %s", *arg);
failret:
        if (evaluate)
            dict_free(d, TRUE);
        return FAIL;
    ...
    int         copyID;
{
    ...
    if (recurse >= DICT_MAXNEST)
    {
        if (!did_echo_string_emsg)
        {
            ...
            EMSG((char *)"E724: variable nested too deep for displaying");
        }
        *tofree = NULL;
        return (char_u *)"{E724}";
    }
    ...
    switch (tv->v_type)
    {
        ...
        default:
            EMSG2((char *)e_intern2, "echo_string()");
            *tofree = NULL;
    }
    ...
    int         copyID;
{
    switch (tv->v_type)
    {
        ...
        case VAR_DICT:
            break;
        default:
            EMSG2((char *)e_intern2, "tv2string()");
    }
 ...
 */
    static int
get_func_tv(name, len, rettv, arg, firstline, lastline, doesrange, evaluate, selfdict)
    char_u      *name;          /* name of the function */
 ...
 */
    static int
call_func(funcname, len, rettv, argcount, argvars, firstline, lastline, doesrange, evaluate, selfdict)
 ...
 * "ermsg" is to be passed without translation, use  instead of _().
 */
    static void
    ...
    float_T     *f;
{
    ...
    EMSG((char *)"E808: Number or Float required");
    return FAIL;
}
    ...
    typval_T    *rettv;
{
    ...
    if (argvars[0].v_type == VAR_LIST)
    {
        if ((l = argvars[0].vval.v_list) != NULL
                && !tv_check_lock(l->lv_lock, (char_u *)"add() argument")
    ...
    typval_T    *avar;
{
    ...
    else if (avar->v_type == VAR_STRING && avar->vval.v_string != NULL)
    {
        ...
        if (buf == NULL)
        {
            ...
            for (buf = firstbuf; buf != NULL; buf = buf->b_next)
                if (buf->b_fname != NULL && (path_with_url(buf->b_fname)) && STRCMP(buf->b_fname, avar->vval.v_string) == 0)
                    break;
        }
    }
    return buf;
    ...
    typval_T    *rettv;
{
    ...
    for (item = args->vval.v_list->lv_first; item != NULL; item = item->li_next)
    {
        ...
        {
            EMSG((char *)"E699: Too many arguments");
            break;
        }
    ...
    char_u      *action;
{
    ...
    for (hi2 = d2->dv_hashtab.ht_array; todo > 0; ++hi2)
    {
        if (!HASHITEM_EMPTY(hi2))
        {
            ...
            {
                EMSG2((char *)"E737: Key already exists: %s", hi2->hi_key);
                break;
            }
    ...
    typval_T    *rettv;
{
    ...
    if (argvars[0].v_type == VAR_LIST && argvars[1].v_type == VAR_LIST)
    {
        ...
        if (l1 != NULL && !tv_check_lock(l1->lv_lock, (char_u *)arg_errmsg) && l2 != NULL)
        {
            if (argvars[2].v_type != VAR_UNKNOWN)
            {
    ...
    else if (argvars[0].v_type == VAR_DICT && argvars[1].v_type == VAR_DICT)
    {
        ...
        if (d1 != NULL && !tv_check_lock(d1->dv_lock, (char_u *)arg_errmsg) && d2 != NULL)
        {
        ...
        EMSG2((char *)e_listdictarg, "extend()");
}
    ...
    int         map;
{
    ...
    if (argvars[0].v_type == VAR_LIST)
    {
        if ((l = argvars[0].vval.v_list) == NULL || tv_check_lock(l->lv_lock, (char_u *)arg_errmsg))
            return;
    }
    else if (argvars[0].v_type == VAR_DICT)
    {
        if ((d = argvars[0].vval.v_dict) == NULL || tv_check_lock(d->dv_lock, (char_u *)arg_errmsg))
            return;
    }
    else
    {
        EMSG2((char *)e_listdictarg, ermsg);
        return;
    }
    ...
    if (expr != NULL)
    {
        ...
        if (argvars[0].v_type == VAR_DICT)
        {
            ...
            for (hi = ht->ht_array; todo > 0; ++hi)
            {
                if (!HASHITEM_EMPTY(hi))
                {
                    ...
                    --todo;
                    di = HI2DI(hi);
                    if (tv_check_lock(di->di_tv.v_lock, (char_u *)arg_errmsg))
                        break;
        ...
        else
        {
            ...
            {
                if (tv_check_lock(li->li_tv.v_lock, (char_u *)arg_errmsg))
                    break;
                nli = li->li_next;
    ...
    typval_T    *rettv;
{
    ...
    if (s == NULL || *s == NUL || VIM_ISDIGIT(*s))
        EMSG2((char *)e_invarg2, s);
    /* Don't check an autoload name for existence here. */
    else if (vim_strchr(s, AUTOLOAD_CHAR) == NULL && !function_exists(s))
        EMSG2((char *)"E700: Unknown function: %s", s);
    else
    {
    ...
    tabpage_T   *tp UNUSED;     /* NULL for current tab page */
{
    ...
    for (wp = (tp == NULL || tp == curtab) ? firstwin : tp->tp_firstwin; wp != NULL; wp = wp->w_next)
        if (--nr <= 0)
            break;
    return wp;
}
    ...
    typval_T    *rettv UNUSED;
{
    ...
    if (str == NULL)
        ...
        n = del_history_idx(get_histtype(str), (int)get_tv_number(&argvars[1]));
    else
        /* string given: remove all matching entries */
        n = del_history_entry(get_histtype(str), get_tv_string_buf(&argvars[1], buf));
    rettv->vval.v_number = n;
}
    ...
    int         inputdialog;
{
    ...
    if (prompt != NULL)
    {
        ...
        if (defstr != NULL)
        {
            ...
            ex_normal_busy = 0;
            rettv->vval.v_string =
                getcmdline_prompt(inputsecret_flag ? NUL : '@', p, echo_attr, xp_type, xp_arg);
    ...
    typval_T    *rettv;
{
    ...
    {
        EMSG2((char *)e_listarg, "inputlist()");
        return;
    }
    ...
    typval_T    *rettv;
{
    ...
    {
        verb_msg((char_u *)"called inputrestore() more often than inputsave()");
        rettv->vval.v_number = 1; /* Failed */
    }
}
    ...
    typval_T    *rettv;
{
    ...
    if (argvars[0].v_type != VAR_LIST)
        EMSG2((char *)e_listarg, "insert()");
    else if ((l = argvars[0].vval.v_list) != NULL
            && !tv_check_lock(l->lv_lock, (char_u *)"insert() argument"))
    {
        if (argvars[2].v_type != VAR_UNKNOWN)
            before = get_tv_number_chk(&argvars[2], &error);
        if (error)
    ...
    typval_T    *rettv;
{
    ...
    rettv->vval.v_number = -1;
    end = get_lval(get_tv_string(&argvars[0]), NULL, &lv, FALSE, FALSE, GLV_NO_AUTOLOAD, FNE_CHECK_START);
    if (end != NULL && lv.ll_name != NULL)
    {
        ...
        else
        {
            if (lv.ll_tv == NULL)
            {
                ...
                EMSG((char *)"E786: Range not allowed");
            else if (lv.ll_newkey != NULL)
                EMSG2((char *)e_dictkey, lv.ll_newkey);
    ...
    typval_T    *rettv;
{
    switch (argvars[0].v_type)
    {
        ...
        case VAR_DICT:
            ...
            break;
        default:
            EMSG((char *)"E701: Invalid type for len()");
            break;
    }
}
                    ...
                    if (regmatch.endp[i] == NULL)
                    {
                        if (list_append_string(rettv->vval.v_list, (char_u *)"", 0) == FAIL)
                            break;
                                ...
                                (int)(regmatch.endp[i] - regmatch.startp[i])) == FAIL)
                        break;
                }
            }
            else if (type == 2)
    ...
    typval_T    *rettv UNUSED;
{
    ...
    if (argvars[1].v_type != VAR_LIST)
    {
        EMSG2((char *)e_listarg, "matchaddpos()");
        return;
    }
    l = argvars[1].vval.v_list;
    if (l == NULL)
    ...
    typval_T    *rettv;
{
    ...
    if (error)
        return;         /* type error; errmsg already given */
    if (stride == 0)
        EMSG((char *)"E726: Stride is zero");
    else if (stride > 0 ? end + 1 < start : end - 1 > start)
        EMSG((char *)"E727: Start past end");
    else
    {
        if (rettv_list_alloc(rettv) == OK)
    ...
    typval_T    *rettv;
{
    ...
    {
        EMSG2((char *)e_notopen, *fname == NUL ? (char_u *)"<empty>" : fname);
        return;
    }
    ...
    typval_T    *rettv;
{
    ...
    {
        if (argvars[2].v_type != VAR_UNKNOWN)
            EMSG2((char *)e_toomanyarg, "remove()");
        else if ((d = argvars[0].vval.v_dict) != NULL
                && !tv_check_lock(d->dv_lock, (char_u *)arg_errmsg))
        {
        ...
        EMSG2((char *)e_listdictarg, "remove()");
    else if ((l = argvars[0].vval.v_list) != NULL
            && !tv_check_lock(l->lv_lock, (char_u *)arg_errmsg))
    {
        int         error = FALSE;
    ...
    typval_T    *rettv;
{
    ...
    p = get_tv_string(&argvars[0]);
    {
        ...
        if (p[0] == '.' && (vim_ispathsep(p[1]) || (p[1] == '.' && (vim_ispathsep(p[2])))))
        ...
        for (;;)
        {
            for (;;)
            {
                ...
                if (limit-- == 0)
                {
                    vim_free(p);
                    vim_free(remain);
                    EMSG((char *)"E655: Too many symbolic links (cycle?)");
                    rettv->vval.v_string = NULL;
                    goto fail;
                }
    ...
    typval_T    *rettv;
{
    ...
    if (argvars[0].v_type != VAR_LIST)
        EMSG2((char *)e_listarg, "reverse()");
    else if ((l = argvars[0].vval.v_list) != NULL
            && !tv_check_lock(l->lv_lock, (char_u *)"reverse() argument"))
    {
        li = l->lv_last;
    ...
    int         *flagsp;
{
    ...
    {
        EMSG2((char *)e_invarg2, get_tv_string(&argvars[1]));
        goto theend;
    }
    ...
    pos_T       *match_pos;
{
    ...
    {
        EMSG2((char *)e_invarg2, get_tv_string(&argvars[3]));
        goto theend;
    }
 ...
 */
    long
do_searchpair(spat, mpat, epat, dir, skip, flags, match_pos, lnum_stop, time_limit)
    ...
    long        time_limit UNUSED; /* stop after this many msec */
{
    ...
    if (*mpat == NUL)
        ...
        sprintf((char *)pat3, "\\(%s\\m\\)\\|\\(%s\\m\\)\\|\\(%s\\m\\)", spat, epat, mpat);
    if (flags & SP_START)
        options |= SEARCH_START;
    ...
    for (;;)
    {
        n = searchit(curwin, curbuf, &pos, dir, pat, 1L, options, RE_SEARCH, lnum_stop, &tm);
    ...
    int         sort;
{
    ...
    if (argvars[0].v_type != VAR_LIST)
        EMSG2((char *)e_listarg, sort ? "sort()" : "uniq()");
    else
    {
        l = argvars[0].vval.v_list;
        if (l == NULL || tv_check_lock(l->lv_lock,
               (char_u *)(sort ? (char *)"sort() argument" : (char *)"uniq() argument")))
            return;
        rettv->vval.v_list = l;
        rettv->v_type = VAR_LIST;
        ++l->lv_refcount;
        ...
        if (sort)
        {
            ...
            if (item_compare_func != NULL
                ...
                EMSG((char *)"E702: Sort compare function failed");
            else
            {
        ...
        else
        {
            ...
            for (li = l->lv_first; li != NULL && li->li_next != NULL; li = li->li_next)
            {
                ...
                if (item_compare_func_err)
                {
                    EMSG((char *)"E882: Uniq compare function failed");
                    break;
                }
            }
    ...
    int         retlist;
{
    ...
    if (argvars[1].v_type != VAR_UNKNOWN)
    {
        ...
        if (err)
        {
            EMSG((char *)"E677: Error writing temp file");
            goto errret;
        }
    }
    ...
    typval_T    *rettv;
{
    ...
    switch (argvars[0].v_type)
    {
        ...
        default: EMSG2((char *)e_intern2, "f_type()"); n = 0; break;
    }
    rettv->vval.v_number = n;
}
    ...
    typval_T    *rettv;
{
    ...
    if (argvars[0].v_type != VAR_LIST)
    {
        EMSG2((char *)e_listarg, "writefile()");
        return;
    }
    if (argvars[0].vval.v_list == NULL)
        return;
    ...
    {
        EMSG2((char *)e_notcreate, *fname == NUL ? (char_u *)"<empty>" : fname);
        ret = -1;
    }
    else
    {
    ...
    char_u      *oldarg;
{
    ...
    if (eap->force_enc != 0)
        sprintf((char *)newval + STRLEN(newval), " ++enc=%s", eap->cmd + eap->force_enc);
    ...
    int         *denote;
{
    ...
    switch (varp->v_type)
    {
        ...
        case VAR_FLOAT:
            EMSG((char *)"E805: Using a Float as a Number");
            break;
        case VAR_FUNC:
            EMSG((char *)"E703: Using a Funcref as a Number");
            break;
        case VAR_STRING:
            if (varp->vval.v_string != NULL)
                vim_str2nr(varp->vval.v_string, NULL, NULL, TRUE, TRUE, &n, NULL);
            return n;
        case VAR_LIST:
            EMSG((char *)"E745: Using a List as a Number");
            break;
        case VAR_DICT:
            EMSG((char *)"E728: Using a Dictionary as a Number");
            break;
        default:
            EMSG2((char *)e_intern2, "get_tv_number()");
            break;
    }
    ...
    char_u      *buf;
{
    switch (varp->v_type)
    {
        case VAR_NUMBER:
            ...
            return buf;
        case VAR_FUNC:
            EMSG((char *)"E729: using Funcref as a String");
            break;
        case VAR_LIST:
            EMSG((char *)"E730: using List as a String");
            break;
        case VAR_DICT:
            EMSG((char *)"E731: using Dictionary as a String");
            break;
        case VAR_FLOAT:
            EMSG((char *)e_float_as_string);
            break;
    ...
    int         no_autoload;
{
    ...
    if (*varname == NUL)
    {
        ...
        switch (htname)
        {
            ...
            case 'l': return current_funccal == NULL ? NULL : &current_funccal->l_vars_var;
            case 'a': return current_funccal == NULL ? NULL : &current_funccal->l_avars_var;
        }
        return NULL;
    }
    ...
    int         *first;
{
    ...
    list_one_var_a(prefix, v->di_key, v->di_tv.v_type, s == NULL ? (char_u *)"" : s, first);
    vim_free(tofree);
}
    ...
    int         copy;       /* make copy of value in "tv" */
{
    ...
    if (v != NULL)
    {
        ...
        if (var_check_ro(v->di_flags, name) || tv_check_lock(v->di_tv.v_lock, name))
            return;
        ...
        {
            EMSG2((char *)"E706: Variable type mismatch for: %s", name);
            return;
        }
    ...
    char_u      *name;
{
    ...
    {
        EMSG2((char *)e_readonlyvar, name);
        return TRUE;
    }
    if ((flags & DI_FLAGS_RO_SBX) && sandbox)
    {
        EMSG2((char *)e_readonlysbx, name);
        return TRUE;
    }
    return FALSE;
}
    ...
    char_u      *name;
{
    ...
    {
        EMSG2((char *)"E795: Cannot delete variable %s", name);
        return TRUE;
    }
    return FALSE;
}
    ...
    int    new_var;  /* TRUE when creating the variable */
{
    ...
    {
        EMSG2((char *)"E704: Funcref variable name must start with a capital: %s", name);
        return TRUE;
    }
    ...
    {
        EMSG2((char *)"E705: Variable name conflicts with existing function: %s", name);
        return TRUE;
    }
    return FALSE;
}
    ...
    char_u      *name;
{
    if (lock & VAR_LOCKED)
    {
        EMSG2((char *)"E741: Value is locked: %s",
                                name == NULL ? (char_u *)"Unknown" : name);
        return TRUE;
    }
    if (lock & VAR_FIXED)
    {
        EMSG2((char *)"E742: Cannot change value of %s",
                                name == NULL ? (char_u *)"Unknown" : name);
        return TRUE;
    }
    return FALSE;
    ...
    int         copyID;
{
    ...
    if (recurse >= DICT_MAXNEST)
    {
        EMSG((char *)"E698: variable nested too deep for making a copy");
        return FAIL;
    }
    ++recurse;
    ...
    switch (from->v_type)
    {
        ...
        case VAR_DICT:
            ...
            break;
        default:
            EMSG2((char *)e_intern2, "item_copy()");
            ret = FAIL;
    }
    --recurse;
    return ret;
    ...
    exarg_T     *eap;
{
    ...
    if (*p != '(')
    {
        if (!eap->skip)
        {
            EMSG2((char *)"E124: Missing '(': %s", eap->arg);
            goto ret_free;
        }
    ...
    if (!eap->skip)
    {
        ...
        if (name != NULL)
            ...
            arg = fudi.fd_newkey;
        if (arg != NULL && (fudi.fd_di == NULL || fudi.fd_di->di_tv.v_type != VAR_FUNC))
        {
            if (*arg == K_SPECIAL)
                j = 3;
            else
        ...
        if (fudi.fd_dict != NULL && fudi.fd_dict->dv_scope == VAR_DEF_SCOPE)
            EMSG((char *)"E862: Cannot use g: here");
    }
    ...
    while (*p != ')')
    {
        ...
        else
        {
            ...
            {
                if (!eap->skip)
                    EMSG2((char *)"E125: Illegal argument: %s", arg);
                break;
            }
            ...
            *p = NUL;
            ...
            for (i = 0; i < newargs.ga_len; ++i)
                ...
                {
                    EMSG2((char *)"E853: Duplicate argument name: %s", arg);
                    vim_free(arg);
                    goto erret;
                }
    ...
    for (;;)
    {
        ...
        if (theline == NULL)
        {
            EMSG((char *)"E126: Missing :endfunction");
            goto erret;
        }
    ...
    if (fp == NULL)
    {
        if (fudi.fd_dict == NULL && vim_strchr(name, AUTOLOAD_CHAR) != NULL)
        {
            ...
            if (j == FAIL)
            {
                EMSG2((char *)"E746: Function name does not match script file name: %s", name);
                goto erret;
            }
        }
    ...
    funcdict_T  *fdp;           /* return: info about dictionary used */
{
    ...
    end = get_lval(start, NULL, &lv, FALSE, skip, flags, lead > 2 ? 0 : FNE_CHECK_START);
    if (end == start)
    {
        if (!skip)
            EMSG((char *)"E129: Function name required");
        goto theend;
    }
    ...
    {
        EMSG2((char *)"E128: Function name must start with a capital or \"s:\": %s", start);
        goto theend;
    }
    if (!skip && !(flags & TFN_QUIET))
    {
        ...
        if (cp != NULL && cp < end)
        {
            EMSG2((char *)"E884: Function name cannot contain a colon: %s", start);
            goto theend;
        }
    }
...
eval_fname_script(p)
    char_u      *p;
{
    if (p[0] == '<' && (STRNICMP(p + 1, "SID>", 4) == 0 || STRNICMP(p + 1, "SNR>", 4) == 0))
        return 5;
    if (p[0] == 's' && p[1] == ':')
        return 2;
    return 0;
    ...
    char_u *name;
{
    ...
    p = trans_function_name(&nm, FALSE, TFN_INT|TFN_QUIET|TFN_NO_AUTOLOAD, NULL);
    nm = skipwhite(nm);
    ...
    exarg_T     *eap;
{
    ...
    if (!eap->skip)
    {
        if (fp == NULL)
        {
            EMSG2((char *)e_nofunc, eap->arg);
            return;
        }
        if (fp->uf_calls > 0)
        {
            EMSG2((char *)"E131: Cannot delete function %s: It is in use", eap->arg);
            return;
        }
    ...
    ufunc_T *fp;
{
    ...
    if (HASHITEM_EMPTY(hi))
        EMSG2((char *)e_intern2, "func_free()");
    else
    ...
    char_u      *name;
{
    ...
    if (name != NULL && isdigit(*name))
    {
        fp = find_func(name);
        if (fp == NULL)
            EMSG2((char *)e_intern2, "func_unref()");
    ...
    dict_T      *selfdict;      /* Dictionary for "self" */
{
    ...
    if (depth >= p_mfd)
    {
        EMSG((char *)"E132: Function call depth is higher than 'maxfuncdepth'");
        rettv->v_type = VAR_NUMBER;
        rettv->vval.v_number = -1;
        return;
    }
    ...
    if (sourcing_name != NULL)
    {
        ...
        if (p_verbose >= 12)
        {
            ...
            smsg((char_u *)"calling %s", sourcing_name);
            if (p_verbose >= 14)
            {
    ...
    if (p_verbose >= 12)
    {
        ...
        if (aborting())
            smsg((char_u *)"%s aborted", sourcing_name);
        else if (fc->rettv->v_type == VAR_NUMBER)
            smsg((char_u *)"%s returning #%ld", sourcing_name, (long)fc->rettv->vval.v_number);
        else
        {
            char_u      buf[MSG_BUF_LEN];
            char_u      numbuf2[NUMBUFLEN];
            ...
            if (s != NULL)
            {
                ...
                smsg((char_u *)"%s returning %s", sourcing_name, s);
                vim_free(tofree);
            }
        }
        msg_puts((char_u *)"\n");   /* don't overwrite this either */
    ...
    exarg_T     *eap;
{
    ...
    if (current_funccal == NULL)
    {
        EMSG((char *)"E133: :return not inside a function");
        return;
    }
